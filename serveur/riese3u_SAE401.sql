...
-- Généré le : ven. 07 mars 2025 à 10:16
...
--
-- Base de données : `riese3u_SAE401`
--
...
-- Procédures
--
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `DELETE_FAVORI` (IN `v_id_prod` INT, IN `v_id_us` INT)  NO SQL BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `id_cat_existe` (IN `v_id_cat` INT, OUT `v_id_cat_existe` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `id_col_existe` (IN `v_id_col` INT, OUT `v_id_col_existe` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `id_com_existe` (IN `v_id_com` INT, OUT `v_id_com_existe` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `id_perm_existe` (IN `v_id_perm` INT, OUT `v_id_perm_existe` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `id_prod_existe` (IN `v_id_prod` INT, OUT `v_id_prod_existe` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `id_tail_existe` (IN `v_id_tail` INT, OUT `v_id_tail_existe` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `id_us_existe` (IN `v_id_us` INT, OUT `v_id_us_existe` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `INSERT_FAVORI` (IN `v_id_prod` INT, IN `v_id_us` INT)  DETERMINISTIC NO SQL BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `login_non_existe` (IN `v_login` VARCHAR(20), OUT `v_login_non_existe` BOOLEAN)  NO SQL BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `mail_non_existe` (IN `v_mail` VARCHAR(100), OUT `v_mail_non_existe` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `verifier_date` (IN `v_date` DATE, OUT `v_date_conforme` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `verifier_prix` (IN `v_prix` FLOAT, OUT `v_prix_correct` BOOLEAN)   BEGIN
...
CREATE DEFINER=`riese3u_appli`@`%` PROCEDURE `verifier_qte` (IN `v_qte` INT, OUT `v_qte_correcte` BOOLEAN)   BEGIN
...
--
ALTER TABLE `COMMANDE`
  MODIFY `id_com` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=68;
...
--
ALTER TABLE `USER`
  MODIFY `id_us` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;
COMMIT;
