...
async function getPanier(id_us) {
	return await fetch("https://devweb.iutmetz.univ-lorraine.fr/~riese3u/2A/SAE-4.01_Tag1/serveur/api/getPanier.php", {
		method: "POST",
...
async function getProduit(id_produit) {
	return await fetch("https://devweb.iutmetz.univ-lorraine.fr/~riese3u/2A/SAE-4.01_Tag1/serveur/api/getProduit.php", {
		method: "POST",
		body: new URLSearchParams({
			id_prod: id_produit,
		}),
...
function delButton(id) {
	...
	test.addEventListener("click", (e) => {
		...
		fetch("https://devweb.iutmetz.univ-lorraine.fr/~riese3u/2A/SAE-4.01_Tag1/serveur/api/delPanier.php", {
			method: "POST",
			body: new URLSearchParams({
				id_us: id_us,
				id_prod: id_prod,
...
function modifButton(id) {
	...
	test.addEventListener("click", (e) => {
		...
		fetch("https://devweb.iutmetz.univ-lorraine.fr/~riese3u/2A/SAE-4.01_Tag1/serveur/api/setPanier.php", {
			method: "POST",
			body: new URLSearchParams({
				id_us: id_us,
				id_prod: id_prod,
...
function rempliSelect(select, array, arrayId, def) {
	...
	arrayId = arrayId.filter(
		(value, index) => arrayId.indexOf(value) === index
	);
	array.forEach((element) => {
...
function affichePanier(panier, qte, taille, couleur, couleurId, tailleId) {
	...
	panierDiv.innerHTML = `
        ...
        <center><img id="img${id}" src="https://devweb.iutmetz.univ-lorraine.fr/~riese3u/2A/SAE-4.01_Tag1/serveur/img/articles/${
		panier.path_img
	...
	prix.innerHTML =
		Math.round(
			(parseFloat(prix.innerHTML) + panier.prix_unit * qte) * 100
		) / 100;
				...
				BDDproduit.data.forEach((element) => {
					if (element.nom_col === e.target.value) {
						document.getElementById(
							`img${id}`
						).src = `https://devweb.iutmetz.univ-lorraine.fr/~riese3u/2A/SAE-4.01_Tag1/serveur/img/articles/${element.path_img}`;
					}
				});
			});
		});
...
window.payer = function () {
	fetch("https://devweb.iutmetz.univ-lorraine.fr/~riese3u/2A/SAE-4.01_Tag1/serveur/api/payer.php", {
		method: "POST",
		body: new URLSearchParams({
			id_us: id_us,
		}),
		...
		reponse.json().then((data) => {
			if (data.status == "success") {
				...
				fetch("https://devweb.iutmetz.univ-lorraine.fr/~riese3u/2A/SAE-4.01_Tag1/serveur/api/clearPanier.php", {
					method: "POST",
					body: new URLSearchParams({
						id_us: id_us,
					}),
